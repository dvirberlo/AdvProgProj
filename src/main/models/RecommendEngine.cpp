...
std::map<int, int> RecommendEngine::computeSharedMovies(const std::map<int, std::set<int>> &dataMap, int targetUserId)
{
    ...
    // Calculate similarity (shared movies) for the target user with all other users
    ...
    userSimilarityMap.erase(targetUserId); // Remove the target user from the map
...
std::vector<int> RecommendEngine::WhoWatchedMovie(const std::map<int, std::set<int>> &dataMap, int targetMovieId)
{
    ...
    // Iterate over each user and their watched movies
     for (const std::pair<const int, std::set<int>>& entry : dataMap) {
        const int userId = entry.first;
        const std::set<int>& movies = entry.second;
        // Check if the targetMovieId is in the set of movies that the user has watched
...
std::map<int, int> RecommendEngine::SumOfSharedMovies(const std::map<int, std::set<int>>& dataMap, int movieId, int userId) {
    //Get a list of users who watched the target movie (movieId)
    //for example: 1 ,2 ,5 ,8
    std::vector<int> usersWhoWatched = WhoWatchedMovie(dataMap, movieId);

    // Compute the number of shared movies of the target user (userId) with all other users
    ...
    for (int curMovie : unionMovies) {
        ...
        for (int watcherId : intersection) {
            if (watcherId != userId) {  // Exclude the target user from the total
                totalSharedMovies += sharedMoviesMap[watcherId];  // Add the number of shared movies
            }
        }
