...
const getRecommendations = async (req, res) => {
    ...
    try {
      ...
      if (user == null) {
        return res.status(404).json({ error: 'The specified user id does not exist' });
      }
    ...
    try {
      ...
      if (movie == null) {
        return res.status(404).json({ error: 'The specified movie id does not exist' });
      }
    ...
    try {
      ...
      if (recommendations.startsWith('200 OK\n')) {
        const cleanedResponse = recommendService.parseRecommendations(recommendations);
  ...
  const addWatch = async (req, res) => {
     ...
     * TODO: get userId from a global file
     */
    const userId = req.headers['token-id'];  // Get userId from token header
    if (!userId) {
      return res.status(400).json({ error: 'Token-ID header is missing' });
    }
    ...
    try {
      ...
      if (addWatchResponse === '201 Created\n') {
        ...
        return res.status(201).json({watcher: `${userId}`, movie: `${req.params.id}`});
