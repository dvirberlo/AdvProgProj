...
const getRecommendations = async (req, res) => {
  try {
    ...
    if (recommendations.startsWith("200 OK\n")) {
      ...
      const cleanedResponse = recommendations
        .replace(/^200 OK\n\n?/, "")
        .trim();
...
const addWatch = async (req, res) => {
  try {
    ...
    var addWatchResponse = await recommendService.postWatch(
      userId,
      req.params.id,
    );
    ...
    if (addWatchResponse === "201 Created\n") {
      return res
        .status(201)
        .json({
          message: `user ${userId} created. movie ${req.params.id} added to watch list`,
        });
