...
const getRecommendations = async (req, res) => {
    ...
    try {
      const recommendations = await recommendService.getRecommendations(userId, movieId);
  ...
  const addWatch = async (req, res) => {
    ...
    try {
      ...
      if (result.status === 201) {
        return res.status(201).json({ watcher: `${userId}`, movie: `${movieId}` });
      } else if (result.status === 204) {
        return res.status(204).json();
      } else {
        return res.status(result.status).json({ error: result.error || result.message });
      }
    } catch (error) {
